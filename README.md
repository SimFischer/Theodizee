...
* Die Spalte `einheit` trennt mehrere Lerneinheiten in derselben Tabelle.
  Diese Anwendung schreibt und liest ausschließlich `luther`. Wird eine zweite
  Einheit aus der Vorlage im selben Supabase-Projekt betrieben, muss die Spalte
  einmalig nachgetragen werden (siehe `supabase_setup.sql`).
